...
// --- COMPONENTE DO MODAL (Pode ser movido para um ficheiro separado depois) ---
const AuthWallModal = ({ onClose }: { onClose: () => void }) => {
    const router = useRouter();
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-6">
            <div className="absolute inset-0 bg-background/60 backdrop-blur-2xl" onClick={onClose} />
            <div className="relative bg-card-custom border border-gold/30 w-full max-w-lg rounded-[40px] p-8 md:p-12 shadow-[0_0_80px_rgba(212,175,55,0.1)] animate-in fade-in zoom-in-95 duration-500">
                <button onClick={onClose} className="absolute top-8 right-8 text-silver-dark hover:text-gold transition-colors">
                    <X size={20} />
                </button>
...
export default function DicionarioFeedPage() {
    ...
    useEffect(() => {
        ...
        // DISPARO AUTOMÁTICO: Se não houver token, mostra o convite após 1.5s
        if (!storedToken) {
            const timer = setTimeout(() => setShowAuthModal(true), 1500);
            loadWords(); // Carrega as palavras em blur/fundo
    ...
    const handleAction = (e: React.MouseEvent, wordId: string) => {
        if (!token) {
            e.preventDefault();
            ...
            <main className="max-w-7xl mx-auto px-6 py-10">
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-40 gap-4">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-[0.4em] text-silver-dark animate-pulse">Sincronizando Acervo</p>
                    </div>
                ) : (
                    <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10 transition-all duration-700 ${!token ? 'blur-sm pointer-events-none select-none opacity-50' : ''}`}>
                        {filteredWords.map((word) => (
                            <div key={word.id} className="relative group cursor-pointer" onClick={(e) => handleAction(e, word.id)}>
                                <div className="block h-full bg-card-custom border border-platinum/20 rounded-[32px] p-8 hover:border-gold/50 transition-all duration-500 shadow-sm hover:shadow-2xl flex flex-col justify-between">
                                    <div>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <p className="text-text-secondary text-lg font-medium leading-relaxed italic italic">&quot;{word.meaning}&quot;</p>
                                        </div>
                                    </div>
                                    <div className="mt-10 pt-6 border-t border-platinum/10 flex justify-between items-center opacity-40 group-hover:opacity-100 transition-opacity">
                                        ...
                                        {!token && <Lock size={12} className="text-gold" />}
                                    </div>
                                </div>
                            </div>
                        ))}
